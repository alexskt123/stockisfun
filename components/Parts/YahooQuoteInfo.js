...
import { convertToPercentage, convertToPriceChange, indicatorVariant, millify, roundTo } from '../../lib/commonFunction'
...
function YahooQuoteInfo({ data, displayQuoteFields }) {
  ...
  function setQuoteDataFields(data) {
    const quoteData = data ? displayQuoteFields.map(item => {
    ...
    return format && format === 'PriceChange' ? value ? getValueBadge(value >= 0 ? 'success' : 'danger', convertToPriceChange(value)) : null
      : format && format === 'PriceChange%' ? value ? getValueBadge(value >= 0 ? 'success' : 'danger', convertToPercentage(value / 100)) : null
        : format && format === 'IndicatorVariant' ? getValueBadge(indicatorVariant(value), value)
          : format && format === 'millify' ? getValueBadge(indicatorVariant(value), millify(value))
            : format && format === 'roundTo' ? getValueBadge(indicatorVariant(value), roundTo(value))
