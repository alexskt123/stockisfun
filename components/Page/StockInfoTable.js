...
import { getRedColor, getGreenColor, getDefaultColor } from '../../lib/commonFunction'
...
const getCellColor = (item, darkMode) => {
  ...
  if (item && item.style && item.style === 'green-red') {
    ...
    return cur < 0 ? { color: getRedColor(darkMode) } : cur > 0 ? { color: getGreenColor(darkMode) } : { color: getDefaultColor(darkMode) }
  }
  else if ((itemData || '').toString().replace(/%/, '') < 0) return { color: getRedColor(darkMode) }
          ...
          <tr key={'tableFirstHeader'}>
            {tableFirstHeader ?
              tableFirstHeader.map((item, index) => (
                <th style={(index == 0 ? sticky : {})} key={index} ><h5><Badge variant="light">{item}</Badge></h5></th>
              ))
              : null}
          </tr>
          <tr key={'tableHeader'}>
            {tableHeader ?
              tableHeader.map((item, index) => (
                <th style={(index == 0 ? Object.assign({...sticky}, darkMode.value ? { backgroundColor: '#343a40' } : {}) : {})} onClick={() => { if (sortItem) sortItem(index) }} key={index} >{item}</th>
              ))
              : null}
          </tr>
        </thead>
        <tbody>
          {tableData ?
            tableData.map((item, index) => (
              <Fragment key={index}>
                <tr>
                  {item.map((xx, yy) => <td onClick={() => { if (cellClick) checkCanClick(item, cellClick) }} style={(yy == 0 ? Object.assign({...sticky}, darkMode.value ? { backgroundColor: '#343a40' } : {}) : {})} key={`${index}${yy}`}><span style={getCellColor(xx, darkMode.value)}>{getCellItem(xx)}</span></td>)}
                </tr>
                {
                  tableDataSkipRow ?
