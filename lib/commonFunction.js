...
import { etfHoldingHeader } from '../config/etf'
...
export const getETFDetail = async (inputTicker) => {
...
export const getFinancialsInfo = async (inputTickers, { tickers, stockInfo }) => {
  ...
  for (const ticker of newTickers) {
    financials = await axios(`/api/yahoo/getYahooFinancials?ticker=${ticker}`)
...
export const getForecastInfo = async (inputTickers, { tickers, stockInfo }) => {
  ...
  for (const ticker of newTickers) {
    forecastResponse = await axios(`/api/forecast/getStockFairValue?ticker=${ticker}`)
    const forecast = {}
...
export const getPriceInfo = async (inputTickers, noOfYears, { tickers, quote, yearlyPcnt, chartData }) => {
  ...
  for (const ticker of newTickers) {
    ...
    outputItem = await axios(`/api/yahoo/getYahooHistoryPrice?ticker=${ticker}&year=${noOfYears}`)
  ...
  const priceInfo = {
    ...
    yearlyPcnt: [...yearlyPcnt,
      ...temp.map(item => {
        const newItem = [
          item.ticker,
          tempQuote.find(x => x.ticker == item.ticker)['Current Price'] || 'N/A',
...
export const getFormattedFromToDate = async (days) => {
  const options = { days }
  const { formattedFromDate, formattedToDate } = getFromToBusDay(options)
