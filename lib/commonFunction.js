...
const getAnnualizedPcnt = (item) => {
  ...
  //exclude 2021
  const totalPcnt = item.data.slice(1).filter(x => x != 'N/A').reduce((acc, data) => {
    acc = acc * (parseFloat(data) / 100 + 1)
    return acc
  }, 1)

  const diffPcnt = convertToPercentage((Math.pow(totalPcnt, 1 / (item.data.filter(x => x !== 'N/A').length - 1)) - 1))
  return diffPcnt
}
...
export const getETFDetailHoldings = async (inputTicker) => {
  ...
  const responses = await Promise.all(holdingInfo.map(async item => {
