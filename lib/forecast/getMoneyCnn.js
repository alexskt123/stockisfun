...
async function getResponse(ticker) {
  const response = await axios
    .get('https://money.cnn.com/quote/forecast/forecast.html', {
      params: {
        symb: ticker
      }
    })
...
export const getMoneyCnn = async ticker => {
  ...
  const moneycnn = spans.reduce(
    (acc, span) => {
      const regAnalyst =
        span.text && span.text.match(/The (.*) analysts offering 12-month/)
      const noOfAnalysts =
        regAnalyst && regAnalyst.length > 1 ? regAnalyst[1] : acc.noOfAnalysts
