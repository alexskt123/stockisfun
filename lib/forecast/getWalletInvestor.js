...
export const getWalletInvestor = async (ticker) => {
  ...
  const walletPercentage = [...Array(2)].map((_item, idx) => {
    return walletInfo[idx + 1] && walletInfo[0] ? percent.calc((walletInfo[idx + 1] - walletInfo[0]), parseFloat(walletInfo[0]), 2, true) : 'N/A'
  })

  const data = {
    ...walletHeader.reduce((acc, item, idx) => {
      return ({...acc,
        [item.item]: [...walletPrice, ...walletPercentage][idx]
      })
    }, {})
  }
