...
export const updUserAllList = async (
  uid,
  { stockList, etfList, watchList, cash }
) => {
  const { id } = await getUserInfoByUID(uid)
...
export const updUserEmailConfig = async (uid, emailConfig) => {
  const { id } = await getUserInfoByUID(uid)
...
const getFileFromStorage = filename => {
  return toAxios(`${process.env.BUCKET_URL}/${filename}`).then(res => res?.data)
}

export const getPug = filename => {
  return getFileFromStorage(`templates/${filename}`)
}
