...
async function getResponse(ticker, fromdate, todate) {
  ...
  const response = validTicker ? await axios(`https://query1.finance.yahoo.com/v8/finance/chart/${quote.ticker}?period1=${fromdate}&period2=${todate}&interval=1d&events=history&=hP2rOschxO0`).catch(console.error)
    : {}
