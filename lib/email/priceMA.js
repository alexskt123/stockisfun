...
const getPriceMADetails = async ({ ticker, genChart }) => {
  ...
  const fromToDate = getFormattedFromToDate(80, true)
  ...
  return users
    .filter(x =>
      x?.emailConfig?.filter(
        x =>
          x.category === 'priceMA' && x?.subscribe && validator.validate(x?.to)
      )
    )
    .reduce(async (acc, user) => {
      ...
      const templates = priceMAEmails
        ...
        .map(email =>
          getHTMLTemplate(user[email.type], email.name, email.subject, email.to)
        )
