...
const getStockStrength = async ticker => {
  ...
  const twentyHigherFifty = conditionMatches('20>50', result.priceMAList)
  const fiftyHigherHundredFifty = conditionMatches('50>150', result.priceMAList)
  const fiftyLowerHundredFifty = conditionMatches('50<150', result.priceMAList)

  return {
    Ticker: result.ticker,
    'Price Not Avail.': !result.priceAvail,
    'RS>0': rs,
...
const getRelativeStrengthByDays = async (
  ...
  const latestHigherInputRange =
    latestPrice.every(x => x) &&
    latestPrice.some(x => earlierPrice.every(e => e < x))
