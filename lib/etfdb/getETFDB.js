...
const handleHoldings = (soup) => {
  ...
  const etfInfo = tds
    .filter(x => x.attrs['data-th'] && x.attrs['data-th'].match(/Symbol|Holding|% Assets/))
    .map(item => item.text)
  ...
  otherPcnt === 100 ? null : etfHolding.push(['Others', 'Others', convertToPercentage(otherPcnt / 100)])
...
const handleNoOfHoldings = (soup) => {
  ...
  const noOfHoldings = divs.reduce((acc, item) => {
    const newAcc = item.text && trim(item.text) === 'Number of Holdings' ? item.findPreviousSibling('div').nextElement.nextElement.text : null
