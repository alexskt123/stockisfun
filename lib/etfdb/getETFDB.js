...
import { convertToPercentage, trim, toAxios } from '@/lib/commonFunction'
...
export const getETFDB = async ticker => {
  ...
  const etfResponse = !validTicker
    ? null
    : await toAxios(`https://etfdb.com/etf/${ticker}`)

  const { data: etfData } = etfResponse || { data: '' }
