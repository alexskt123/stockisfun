...
const handleHoldings = (soup) => {
  ...
  etfHolding.push(['Others', 'Others', `${otherPcnt.toFixed(2)}%`])
...
export const getETFDB = async (ticker) => {
  ...
  const etfRespnose = !validTicker ? { data: '' } : await axios.get(`https://etfdb.com/etf/${ticker}`).catch(error => console.log(error))
  const { data: etfData } = etfRespnose
