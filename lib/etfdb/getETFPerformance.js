...
export const getETFPerformance = async (ticker) => {
  ...
  const etfInfo = spans.filter(span => span && span.text && span.previousElement.previousElement.nextSibling && span.previousElement.previousElement.nextSibling.text.match(/4 Week Return|Year to Date Return|1 Year Return|3 Year Return/)).reduce((acc, span) => {
