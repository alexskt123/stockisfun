...
import { convertToNumber, convertToPercentage, convertFromPriceToNumber, millify, randRGBColor, roundTo } from '../lib/commonFunction'
...
export const getBalanceSheetTableData = (data) => {
  ...
  Object.keys((balanceSheetData.find(x => x) || {})).filter(x => x !== 'Date').forEach(item => {
    const curItem = []
  ...
  return {
    ...
    tableDataSkipRow: ['Total Current Liability', 'Total Non-Current Liability', 'Total Liability', 'Total Stock Holder Equity'],
...
export const getYahooEarnings = (inputData) => {
  ...
  if (inputData) {
    ...
    Object.keys([...inputData].find(x => x) || []).filter(x => x !== 'date').reverse().forEach(item => {
      earnings.tableData.push([item, ...inputData.map(data => millify(data[item]))])
...
export const getYahooBasicsData = (data, quote) => {
  const newData = {
    ...
    'Price%': convertToPercentage(quote.regularMarketChangePercent / 100),
    ...
    'Dividend': quote.trailingAnnualDividendRate ? convertToPercentage(quote.trailingAnnualDividendRate / 100, true) : undefined,
...
export const getYahooBalanceSheetData = (balanceSheet) => {
  const balanceSheetExtract = balanceSheet.map(item => {
    ...
    if (item.totalCurrentAssets && item.totalCurrentLiabilities)
      newItem['Current Ratio'] = roundTo(item.totalCurrentAssets?.raw / item.totalCurrentLiabilities?.raw)

    if (item.totalCurrentLiabilities)
      newItem['Quick Ratio'] = roundTo((item.cash ? item.cash.raw : 0
    ...
    if (item.totalLiab && item.totalStockholderEquity)
      newItem['Total Debt/Equity'] = roundTo(item.totalLiab?.raw / item.totalStockholderEquity?.raw)
    return newItem
  })
